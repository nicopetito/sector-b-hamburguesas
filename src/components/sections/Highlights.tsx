    ...
    <section id="highlights" className="bg-bg-base py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="relative">
          ...
          {/* Fade derecho — hint de scroll, solo mobile */}
          <div className="pointer-events-none absolute right-0 top-0 z-10 h-full w-14 bg-gradient-to-l from-bg-base to-transparent sm:hidden" />
          ...
          >
            {featured.map((item, idx) => (
              <article
                key={item.id}
                className="group relative min-w-[78vw] flex-shrink-0 cursor-pointer overflow-hidden rounded-2xl bg-bg-card snap-start
                           transition-all duration-500 ease-out sm:min-w-0
              ...
              >
                ...
                {/* ── Overlay content ── */}
                <div className="absolute bottom-0 left-0 right-0 p-5 sm:translate-y-1 sm:transition-transform sm:duration-500 sm:ease-out sm:group-hover:translate-y-0">

                  {/* Descripción: oculta en mobile, hover en desktop */}
                  <p className="mb-2 line-clamp-2 hidden text-[0.68rem] leading-relaxed text-white/55
                                sm:block sm:translate-y-2 sm:opacity-0 sm:transition-all sm:duration-400 sm:ease-out
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <a
                      ...
                      className="rounded-full bg-turquesa px-4 py-2 text-[0.7rem] font-bold uppercase tracking-wide text-bg-base
                                 shadow-[0_0_20px_rgba(0,201,177,0.4)] transition-all duration-300
                                 hover:bg-turquesa-dark hover:shadow-[0_0_30px_rgba(0,201,177,0.6)]
                                 sm:opacity-0 sm:group-hover:opacity-100 sm:group-hover:shadow-[0_0_30px_rgba(0,201,177,0.6)]"
                    >
                      Pedir este
                    </a>
                  </div>
        ...
        <div className="mt-5 flex items-center justify-center gap-3 sm:hidden">
          ...
          {/* Swipe hint — desaparece al scrollear */}
          <span
            className={`flex items-center gap-0.5 text-[0.6rem] font-semibold uppercase tracking-widest transition-all duration-500 ${
              activeIdx === 0
          ...
          >
            deslizá
            <ChevronRight size={10} className="animate-bounce-x" strokeWidth={2.5} />
          </span>
        </div>
        ...
        <div className="mt-6 text-center sm:hidden">
          <a
            href="#menu"
            className="text-[0.7rem] font-semibold uppercase tracking-widest text-white/30 transition-colors hover:text-text-primary"
          >
            Ver todas las burgers →
          </a>
        </div>
